...
def sequential_store(n):
    ...
    stats = pd.DataFrame({"Metric":["Access Time","Memory Usage"], "Value":[n*0.5, n*0.1]})
    return df, stats
...
def indexed_store(n):
    ...
    stats = pd.DataFrame({"Metric":["Access Time","Memory Usage"], "Value":[n*0.3, n*0.2]})
    return df, stats
...
def direct_store(n):
    ...
    stats = pd.DataFrame({"Metric":["Access Time","Memory Usage"], "Value":[n*0.2, n*0.25]})
    return df, stats
...
def btree_store(n):
    ...
    stats = pd.DataFrame({"Metric":["Access Time","Memory Usage"], "Value":[n*0.1, n*0.3]})
    return df, stats
